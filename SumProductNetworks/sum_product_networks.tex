...
Let $\X = \{X_1, \dots, X_D\}$ be set of random variables.

A probabilistic circuit over $\X$ is a tuple $\SPN = (\graph, \scope, \tp)$, where
  \begin{itemize}
...
A Sum-Product Network (SPN) is a \emph{smooth} (complete) and \emph{decomposable} probabilistic circuit.
\end{frame}
...
\frame{
\begin{center}Example SPN $\SPN = (\graph, \scope, \tp)$\end{center}
\begin{figure}
  \centering{
...
\begin{frame}{Interpretations of SPNs}
    What is an SPN?
    \begin{itemize}
        \item Multi-linear feed-forward Neural Network (NN) with non-linear inputs.
        \item Deep structured mixture model with exponentially many components.
        \item Hierarchical tensor decomposition.
    \end{itemize}
\end{frame}
