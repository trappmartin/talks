...
\begin{frame}{Bayesian Structure \& Parameter Learning}
  \begin{itemize}
    \item For each dimension $d$ we introduce a latent variable $\fcolorbox{CBRed}{background}{Y}_{\partition,d}$.
    \pause
...
\begin{tikzpicture}
  ...
  \node[foreground, draw=CBYellow, latent, left=of x] (z) {$z_{\SumNode,n}$};
  \node[foreground, draw=CBRed, latent, right=of x] (y) {$y_{\partition,d}$};
  \node[foreground, draw=CBPurple, latent, below=of y] (t) {$\theta_{\Leaf,d}$};
  %\node[const, left=of t] (g) {$\gamma_d$};
  \node[foreground, draw=CBGreen, latent, below=of z] (w) {$w_{\SumNode}$};
  %\node[const, left=of w] (a) {$\alpha$};
  \node[foreground, draw=white, latent, right=of y] (o) {$\vp_{\partition}$};
  %\node[const, right=of o] (b) {$\beta$};
